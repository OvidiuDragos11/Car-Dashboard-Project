...
class _DashBoardMain(QWidget):
    ...
    def customKeyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Left:
            ...
            self.dash_board_design_widget.indicator_trigger(0)
        if event.key() == Qt.Key.Key_Right:
            self.keys_[event.key()] = True
            self.dash_board_design_widget.indicator_trigger(1)
...
class _DashBoardContolsDesign(QWidget):
    ...
    def indicators_properties(self):
        ...
        left_indicator = QPolygonF((QPointF(40, 80), QPointF(90, 120), QPointF(90, 100), QPointF(150, 100),
                                    QPointF(150, 60), QPointF(90, 60), QPointF(90, 40)))
        self.scaled_left_indicator = indicator_trans.map(left_indicator)
        self.scaled_left_indicator.translate(-self.scaled_left_indicator.boundingRect().x(), -self.scaled_left_indicator.boundingRect().y())
        self.scaled_left_indicator.translate(self.width()*0.03, self.height()*0.06)

        # right indicator
        rotate_t = QTransform()
        rotate_t.rotate(180, Qt.Axis.YAxis)
        self.scaled_right_indicator = self.scaled_left_indicator
        self.scaled_right_indicator = rotate_t.map(self.scaled_right_indicator)
        self.scaled_right_indicator.translate(-self.scaled_right_indicator.boundingRect().x(), -self.scaled_right_indicator.boundingRect().y())
        self.scaled_right_indicator.translate(self.width()-self.scaled_right_indicator.boundingRect().width()-self.scaled_left_indicator.boundingRect().x(), self.height()*0.06)
    ...
    def indicators_painting(self, painter: QPainter):
        ...
        painter.drawPolygon(self.scaled_left_indicator)
        ...
        painter.drawPolygon(self.scaled_right_indicator)

    def indicator_trigger(self, indicator):
        if indicator == 0:  # left indicator
            if self.right_indicator_state:
                self.right_indicator_state = False
                self.right_indicator_color = self.indicator_color_list[0]  # Turn off right indicator
            self.left_indicator_state = not self.left_indicator_state
        elif indicator == 1:  # right indicator
            if self.left_indicator_state:
                self.left_indicator_state = False
                self.left_indicator_color = self.indicator_color_list[0]  # Turn off left indicator
    ...
    def indicator_animation(self, pos):
        ...
        # left indicator
        self.scaled_left_indicator.translate(-self.scaled_left_indicator.boundingRect().x(), -self.scaled_left_indicator.boundingRect().y())
        self.scaled_left_indicator.translate(pos, self.height()*0.06)
        # right indicator
        self.scaled_right_indicator.translate(-self.scaled_right_indicator.boundingRect().x(), -self.scaled_right_indicator.boundingRect().y())
        self.scaled_right_indicator.translate(self.width()-self.scaled_right_indicator.boundingRect().width()-self.scaled_left_indicator.boundingRect().x(), self.height()*0.06)
        self.repaint()
...
class _DashBoardControls(QObject):
    ...
    def startup_values_setter(self):
        self.keys_ = self.dash_board.keys_  # original keys
    ...
    def all_connector(self):
        ...
        self.indicator_sig.connect(self.dash_board.dash_board_design_widget.indicator_trigger)
    ...
    def left_indicator_on_or_off(self):
        ...
        self.left_indicator_state = not self.left_indicator_state
        if self.left_indicator_state:
            self.right_indicator_state = False  # Turn off right indicator
    ...
    def right_indicator_on_or_off(self):
        ...
        self.right_indicator_state = not self.right_indicator_state
        if self.right_indicator_state:
            self.left_indicator_state = False  # Turn off left indicator
